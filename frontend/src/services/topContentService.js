...
export const topContentService = {
  ...
  async getTopEpisodes(period = 'all_time', limit = 10, includeImages = false) {
    const response = await fetch(`${API_BASE_URL}/topContent/top/episodes?period=${period}&limit=${limit}&include_images=${includeImages}`);
    if (!response.ok) {
  ...
  async getTopShows(period = 'all_time', limit = 10, includeImages = false) {
    const response = await fetch(`${API_BASE_URL}/topContent/top/shows?period=${period}&limit=${limit}&include_images=${includeImages}`);
    if (!response.ok) {
  ...
  async getAllTopContent(period = 'all_time', limit = 5, includeImages = true) {
